...
for example, having both a neutral and acid mechanism for mineral dissolution.
...
The way it is implemented is as follows:
  ...
  - If the mineral does not have a kinetics label on the input file then the label name will be take as the CrunchTope default, which is 'default'.
    - For example, an entry in the input file of `Foresterite -rate -12.0` would be indexable in a `.yaml` file as `Foresterite_default`.

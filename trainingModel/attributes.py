...
def get_condition(data_set, condition, species_concs=False, mineral_volumes=False, mineral_rates=False):
    ...
    # Currently only supports primary_species but can easily be extended to other attributes of a ConditionBlocks once Omphalos supports their randomisation.
    ...
    for i in data_set:
        ...
        # Get the DataFrames containing attribute info for each condition block part and join them together to make attribute df describing the InputFile.
        row = pd.DataFrame()
        row_parts = []
...
def mineral_volume(input_file, condition):
    # Creat a deep copy of the mineral volume fraction dictionary so that the data wrangling doesn't mangle the InputFile.
    minerals_dict = copy.deepcopy(input_file.condition_blocks[condition].minerals)
...
def primary_species(input_file, condition):
    primary_species_df_row = pd.DataFrame.from_dict(input_file.condition_blocks[condition].primary_species)
